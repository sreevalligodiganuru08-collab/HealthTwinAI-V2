...
function renderSummary(a) {
  ...
  if (!a || !a.avg) {
    el.innerHTML = "<p>No analysis available</p>";
    return;
  }
...
async function analyzeFutureRisk() {
  ...
  records.forEach(r => {
    if (r.heart_rate > 100) {
      ...
      suggestions.push("Reduce stress & caffeine.");
    }

    if (r.spo2 < 95) {
      risks.push("Low Oxygen");
      suggestions.push("Improve breathing.");
    }

    if (r.systolic_bp > 140 || r.diastolic_bp > 90) {
      risks.push("High BP");
      suggestions.push("Exercise & reduce salt.");
    }

    if (r.steps < 3000) {
      risks.push("Low Activity");
      suggestions.push("Walk more daily.");
    }
  });
